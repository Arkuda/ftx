...
internal class SocketMessageManager(private val socket: Socket, private val senderType: SenderType) {
...
//            val rawMessage = receiveChannel.readUTF8Line(calcCharsCountByUTFBytes(receiveChannel.availableForRead)+1) ?: return null
            val decodedMessage = Json.decodeFromString<SocketMessage>(rawMessage)
            return decodedMessage
        } catch (e: Exception) {
            LogManager.log(LogMessage.ExceptionLogMessage("SocketMessageManager ${senderType.name}", "Error while receive message $e",e))
            return null
        }
    }

    private fun calcCharsCountByUTFBytes(availableForRead: Int): Int = availableForRead / 4
    ...
    suspend fun waitMessage(predicate: suspend (SocketMessage) -> Boolean, timeoutInSec: Int = 15): SocketMessage? {
        LogManager.log(LogMessage.StringLogMessage("MESSANGER ${senderType.name} ${socket.localAddress} ${socket.hashCode()}", " wait Message"))
        try {
